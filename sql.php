...
function connexion_mysqli() {
    ...
    try { // Essaie de se connecter à la base de données
        ...
        $_SESSION['error'] = "La connexion à la base de données a échoué : " . $e->getMessage();
        header('Location: login.php'); // Redirige vers la page d'erreur
        exit();
    }
    ...
    if ($connexion->connect_error) { // Si la connexion a échoué, stocke un message d'erreur
        $_SESSION['error'] = "La connexion à la base de données a échoué : " . $connexion->connect_error;
        header('Location: login.php'); // Redirige vers la page d'erreur
        exit();
    }
